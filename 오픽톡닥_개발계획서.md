...
| 페이지 | 경로 | 설명 |
|--------|------|------|
| 시험후기 메인 | `/reviews` | 3탭 구조 (빈도 분석 / 후기 제출 / 시험 후기) |

> UI 셸 구현 완료 (2026-02-20). 탭 내부 기능은 백엔드 연동 후 구현.
> 기존 데이터랩의 5개 별도 페이지를 1페이지 3탭 구조로 통합.
...
### Step 2-b: 내 스크립트 (스크립트 모듈에 통합)

> 생성된 스크립트 3패널 관리 + 오디오 동기화 학습.
> 스크립트 모듈의 "내 스크립트" 탭에서 접근.
> 📖 분석결과 §10 참조
...
### Step 2-c: 쉐도잉 (스크립트 모듈에 통합)

> 패키지 기반 4모드 쉐도잉 훈련. Whisper STT + GPT-4 평가.
> 스크립트 모듈의 "쉐도잉 훈련" 탭에서 접근.
> 📖 분석결과 §11 참조
...
| Step | 모듈 | 상태 | 비고 |
|------|------|------|------|
| Step 0 | 핵심 코어 | ⏳ 대기 | master_questions + custom_mode_questions |
| Step 1 | 시험후기 | 🔄 UI 셸 완료 | /reviews 3탭 구조, 백엔드 미연동 |
| Step 2 | 스크립트 | ⏳ 대기 | 생성 + 내 스크립트 + 쉐도잉 통합 |
| Step 3 | 모의고사 | ⏳ 대기 | — |
| Step 4 | 튜터링 | ⏳ 대기 | 구 AI 튜터 |
...
*최종 업데이트: 2026-02-20*
*상태: Phase 3 진입 — 시험후기 UI 셸 완료, Step 0 (핵심 코어 DB 설계) 대기*
