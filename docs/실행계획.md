...
Phase 2 (랜딩/필수 페이지) ✅ 완료
    ↓
Phase 3 (핵심 모듈 이관) ← 현재 단계 (Step 2 완료, Step 3 대기)
    ↓
...
| 쉐도잉 훈련 | `/scripts/shadowing` | 5단계 점진 훈련 (듣기/오버래핑/쉐도잉/낭독/실전) |
...
Step 1: 시험후기 ✅ 완료 (Server Actions only)
    ↓
Step 2: 스크립트+쉐도잉 🔄 거의 완료 (위저드+뷰어+EF 완료, 패키지 EF+쉐도잉 UI 남음)
    ↓
...
| RLS 정책 + 인덱스 8개 + RPC 함수 2개 + Storage 버킷 | ✅ |
| ai_prompt_templates 시드 2행 + script_specs 시드 60행 | ✅ |
| TypeScript 타입 + Zod 스키마 7개 | ✅ |
| Server Actions 11개 | ✅ |
| Edge Function generate/correct/refine — Two-Pass 아키텍처 (Pass 1 생성 + Pass 2 학습분석 리스트) | ✅ |
| EF Supabase 배포 + 테스트 완료 (200 OK, 31초) | ✅ |
| 5단계 위저드 (질문선택 → 옵션설정 → opic_tips 대기화면 → 결과확인 → 확정) | ✅ |
...
| # | 항목 | 상태 | 설명 |
|---|------|------|------|
| 1 | 패키지 생성 EF | ⏳ | scripts-package EF (ElevenLabs TTS + Whisper 타임스탬프) |
...
└── functions/scripts/index.ts            (생성)
frontend/
├── lib/
│   ├── types/scripts.ts                  (생성)
│   ├── validations/scripts.ts            (생성)
│   ├── actions/scripts.ts                (생성)
│   └── queries/master-questions.ts       (수정: +getTopics, +getQuestionsByCategory)
├── components/scripts/
│   ├── scripts-content.tsx               (수정: TanStack Query 통합)
│   └── create/
│       ├── script-wizard.tsx             (생성: 5단계 위저드)
...
- **쉐도잉 5단계 점진 훈련**: 듣기/오버래핑/쉐도잉(클라이언트) + 낭독(클라이언트) + 실전(API)
- 소리담 4모드가 아닌 학술 BP 기반 신규 설계 (Kadota 2019 + ELT School 5단계)

#### 미완료 항목 (Phase 3 이후 또는 E2E 테스트 시)
- Edge Function Supabase 배포 (OPENAI_API_KEY 환경변수 필요)
- 클라이언트 → Edge Function 호출 연결 (스크립트 생성 후 AI 호출)
- 패키지 생성 (ElevenLabs TTS + Whisper 타임스탬프) Edge Function
- 쉐도잉 훈련 페이지 (/scripts/shadowing) 상세 UI
- 쉐도잉 평가 Edge Function (Whisper STT + GPT 평가)
...
| **Phase 3 (핵심 모듈)** | **🔄 진행 중** | **Step 2 UX 고도화 완료, 패키지 EF + 쉐도잉 UI 남음** |
...
| **Step 2** | **스크립트+쉐도잉** | **🔄 거의 완료** | **SA + EF** | 6테이블+11SA+3EF+위저드+뷰어 완료. 남은: 패키지 생성 EF + 쉐도잉 훈련 UI |
...
| `ai_prompt_templates` | 2행 | RCTF System+User Prompt (Step 2) |
...
*최종 업데이트: 2026-02-26*
*상태: Phase 3 Step 2 거의 완료 (패키지 EF + 쉐도잉 UI 남음) → 리브랜딩(P-5) → Step 3 모의고사*
